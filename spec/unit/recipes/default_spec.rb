...
describe 'letsencryptaws::default' do
  ...
  it 'downloads default certificates' do
    expect(chef_run).to create_s3_file('/etc/ssl/certs/default.crt')
    expect(chef_run).to create_s3_file('/etc/ssl/private/default.key')
    expect(chef_run).to create_s3_file('/etc/ssl/certs/default.ca')
  end
  ...
  it 'downloads requested certificates' do
    expect(chef_run).to create_s3_file('/etc/ssl/certs/test.example.com.crt')
    expect(chef_run).to create_s3_file('/etc/ssl/private/test.example.com.key')
    expect(chef_run).to create_s3_file('/etc/ssl/certs/test.example.com.ca')
  end
  ...
  it 'generates pkcs12 keyring' do
    ...
    expect(chef_run.execute('generate pkcs12 store for test.example.com')).to \
      subscribe_to('s3_file[/etc/ssl/certs/test.example.com.crt]').on(:run).delayed
